...
LNS::LNS(const Instance& instance, double time_limit, Time::time_point budget_start_time,
         ...
         screen(screen), start_time(budget_start_time), path_table(instance.map_size),pipp_option(pipp_option),
         replan_time_limit(replan_time_limit)
{
    const double cleanup_seconds = min(0.5, time_limit * 0.02);
    search_deadline = start_time + duration_cast<Time::duration>(
            duration<double>(max(0.0, time_limit - cleanup_seconds)));
    const auto preprocessing_start = Time::now();
    ...
    preprocessing_time = ((fsec)(Time::now() - preprocessing_start)).count();
    if (screen >= 2)
...
bool LNS::run()
{
    // Used only for statistics, but still charged to the end-to-end cutoff.
    sum_of_distances = 0;
    for (const auto & agent : agents)
    {
        sum_of_distances += agent.path_planner.my_heuristic[agent.path_planner.start_location];
    }
    // The cutoff covers the complete C++ workload.  start_time originates in
    // driver.cpp before Instance construction, rather than being reset here.
    initial_solution_runtime = ((fsec)(Time::now() - start_time)).count();
    bool succ = has_external_initial_solution;
    int count = 0;
    while (!succ && Time::now() < search_deadline)
    {
    ...
    writeIncumbentPaths();
    if (trace)
        cout << "LNS_TRACE runtime=" << initial_solution_runtime
             << " soc=" << initial_sum_of_costs << endl << std::flush;
    ...
    double last_improvement_runtime =
            has_external_initial_solution ? 0 : initial_solution_runtime;
    while (Time::now() < search_deadline && iteration_stats.size() <= num_of_iterations)
    {
        runtime =((fsec)(Time::now() - start_time)).count();
        if (early_stop_seconds > 0 &&
            runtime - last_improvement_runtime >= early_stop_seconds)
            break;
        const int previous_sum_of_costs = sum_of_costs;
        if(screen >= 1)
            validateSolution();
        if (ALNS)
        ...
        if (sum_of_costs < previous_sum_of_costs)
            last_improvement_runtime = runtime;
        if (screen >= 1)
        ...
        writeIncumbentPaths();
        if (trace)
            cout << "LNS_TRACE runtime=" << runtime
                 << " soc=" << sum_of_costs << endl << std::flush;
    }
...
bool LNS::loadInitialSolution(const string& file_name)
{
    std::ifstream input(file_name);
    if (!input.is_open())
    {
        cerr << "Initial solution file " << file_name << " not found." << endl;
        return false;
    }

    int num_agents = 0;
    input >> num_agents;
    if (!input.good() || num_agents != (int)agents.size())
    {
        cerr << "Initial solution file contains " << num_agents
             << " agents, but the instance has " << agents.size() << " agents." << endl;
        return false;
    }
    ...
    for (int i = 0; i < num_agents; i++)
    {
        int agent_id = -1;
        int path_length = 0;
        input >> agent_id >> path_length;
        if (!input.good() || agent_id < 0 || agent_id >= (int)agents.size() || path_length <= 0)
        {
            cerr << "Malformed initial path entry for row " << i << "." << endl;
            return false;
        }

        neighbor.agents[agent_id] = agent_id;
        agents[agent_id].path.clear();
        agents[agent_id].path.reserve(path_length);
        for (int t = 0; t < path_length; t++)
        {
            int location = -1;
            input >> location;
            if (!input.good() || location < 0 || location >= instance.map_size || instance.isObstacle(location))
            {
                cerr << "Invalid location in initial path for agent " << agent_id
                     << " at timestep " << t << "." << endl;
                return false;
            }
            if (t > 0 && !instance.validMove(agents[agent_id].path.back().location, location))
            {
                cerr << "Invalid move in initial path for agent " << agent_id
                     << " at timestep " << t << "." << endl;
                return false;
            }
        ...
        if (agents[agent_id].path.front().location != agents[agent_id].path_planner.start_location ||
            agents[agent_id].path.back().location != agents[agent_id].path_planner.goal_location)
        {
            cerr << "Initial path for agent " << agent_id
                 << " does not match the instance start/goal." << endl;
            return false;
        }

        path_table.insertPath(agent_id, agents[agent_id].path);
        neighbor.sum_of_costs += (int)agents[agent_id].path.size() - 1;
    }

    initial_sum_of_costs = neighbor.sum_of_costs;
    sum_of_costs = neighbor.sum_of_costs;
    has_external_initial_solution = true;
    init_algo_name = "External";
    return true;
}
...
bool LNS::runEECBS()
{
    ...
    runtime = ((fsec)(Time::now() - start_time)).count();
    double T = max(0.0f, ((fsec)(search_deadline - Time::now())).count());
...
bool LNS::runPP()
{
    ...
    while (p != shuffled_agents.end() && ((fsec)(Time::now() - time)).count() < T)
    {
        ...
        agents[id].path = agents[id].path_planner.findOptimalPath(
                path_table, search_deadline, timed_out);
        if (timed_out)
        {
...
bool LNS::runPPS(){
    ...
    PPS solver(&P,MT_S);
    solver.setTimeLimit(max(0.0f, ((fsec)(search_deadline - Time::now())).count()));
...
void LNS::validateSolution() const
{
    ...
    for (const auto& agent : agents)
    {
        if (agent.path.empty())
        {
            cerr << "No solution for agent " << agent.id << endl;
            exit(-1);
        }
        if (agent.path_planner.start_location != agent.path.front().location)
        {
            cerr << "The path of agent " << agent.id << " starts from location " << agent.path.front().location
                << ", which is different from its start location " << agent.path_planner.start_location << endl;
            exit(-1);
        }
        if (agent.path_planner.goal_location != agent.path.back().location)
        {
            cerr << "The path of agent " << agent.id << " ends at location " << agent.path.back().location
                 << ", which is different from its goal location " << agent.path_planner.goal_location << endl;
            exit(-1);
        }
        for (int t = 1; t < (int) agent.path.size(); t++)
        {
            ...
            {
                cerr << "The path of agent " << agent.id << " jump from "
                     << agent.path[t - 1].location << " to " << agent.path[t].location
                     << " between timesteps " << t - 1 << " and " << t << endl;
                exit(-1);
            }
        }
        sum += (int) agent.path.size() - 1;
        makespan = max(makespan, (int) agent.path.size() - 1);
    }

    vector<int> occupied(instance.map_size, -1);
    vector<int> touched;
    touched.reserve(agents.size());
    std::unordered_map<uint64_t, int> traversed;
    traversed.reserve(agents.size());
    for (int t = 0; t <= makespan; t++)
    {
        ...
        for (const auto& agent : agents)
        {
            const int index = min(t, (int) agent.path.size() - 1);
            const int location = agent.path[index].location;
            if (occupied[location] >= 0)
            {
                cerr << "Find a vertex conflict between agents " << occupied[location]
                     << " and " << agent.id << " at location " << location
                     << " at timestep " << t << endl;
                exit(-1);
            }
            occupied[location] = agent.id;
            touched.push_back(location);

            if (t > 0)
            {
                const int previous = agent.path[min(t - 1, (int) agent.path.size() - 1)].location;
                if (previous != location)
                {
                    const uint64_t reverse = (uint64_t(location) << 32) | uint32_t(previous);
                    const auto conflict = traversed.find(reverse);
                    if (conflict != traversed.end())
                    {
                        cerr << "Find an edge conflict between agents " << conflict->second
                             << " and " << agent.id << " at edge (" << previous << ","
                             << location << ") at timestep " << t << endl;
                        exit(-1);
                    }
                    const uint64_t edge = (uint64_t(previous) << 32) | uint32_t(location);
                    traversed.emplace(edge, agent.id);
                }
            }
        }
        for (int location : touched)
            occupied[location] = -1;
    }
    if (sum_of_costs != sum)
    {
...
void LNS::writeIncumbentPaths() const
{
    if (incumbent_path_file.empty())
        return;
    const auto tmp_file = incumbent_path_file + ".tmp";
    writePathsToFile(tmp_file);
    std::rename(tmp_file.c_str(), incumbent_path_file.c_str());
}
/*
