...
Planner::Planner(const Instance *_ins, int _verbose, Deadline *_deadline,
                 int _seed, int _depth, DistTable *_D,
                 double _post_solution_time_limit_ms)
    : ins(_ins),
      deadline(_deadline),
      seed(_seed),
      post_solution_time_limit_ms(_post_solution_time_limit_ms),
      MT(std::mt19937(seed)),
      verbose(_verbose),
      depth(_depth),
      N(ins->N),
...
Solution Planner::solve()
{
  ...
  while (!OPEN.empty() && !is_expired(deadline)) {
    ...
    if (H_goal == nullptr && is_same_config(H->C, ins->goals)) {
      ...
      H_goal = H;
      initial_solution = backtrack(H_goal);
      trace.emplace_back(
          time_initial_solution, get_sum_of_costs(initial_solution));
      if (deadline != nullptr && post_solution_time_limit_ms >= 0)
        deadline->reset(post_solution_time_limit_ms);
...
void Planner::rewrite(HNode *H_from, HNode *H_to)
{
  ...
  while (!Q.empty()) {
    ...
    for (auto n_to : n_from->neighbor) {
      ...
      if (g_val < n_to->g) {
        const auto goal_updated = n_to == H_goal;
        if (goal_updated) {
        ...
        n_to->parent = n_from;
        if (goal_updated) {
          const auto soc = get_sum_of_costs(backtrack(H_goal));
          if (soc < trace.back().second)
            trace.emplace_back(elapsed_ms(deadline), soc);
        }
        Q.push(n_to);
